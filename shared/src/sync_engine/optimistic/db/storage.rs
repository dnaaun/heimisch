...
use typed_db::{Db, DbBuilder, RawDbTrait, ReadOnly, ReadWrite, Table};
...
pub struct DbWithOptimisticChanges<RawDb: RawDbTrait> {
    inner: Db<RawDb>,
    optimistic_updates: Arc<OptimisticChanges>,
    pub(crate) listener: CommitListener,
}

impl<RawDb: RawDbTrait> DbWithOptimisticChanges<RawDb> {
    #[track_caller]
    pub async fn new(
        inner: DbBuilder<RawDb>,
...
impl<RawDb: RawDbTrait> DbWithOptimisticChanges<RawDb> {
    #[track_caller]
    pub fn txn(&self) -> TxnBuilderWithOptimisticChanges<'_, RawDb, ReadOnly> {
