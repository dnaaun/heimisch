...
impl CheckRun {
    ...
    pub fn new(
        ...
        output: models::CheckRunOutput,
        name: String,
        check_suite: Option<models::CheckRunCheckSuite>,
        app: Option<models::NullableIntegration>,
        pull_requests: Vec<models::PullRequestMinimal>,
    ) -> CheckRun {
        CheckRun {
            id,
            head_sha,
            ...
            name,
            check_suite: if let Some(x) = check_suite {
                Some(Box::new(x))
            } else {
                None
            },
            app: if let Some(x) = app {
                Some(Box::new(x))
            } else {
                None
            },
