...
impl WebhooksSponsorship {
    pub fn new(
        ...
        sponsor: Option<models::User2>,
        sponsorable: Option<models::User2>,
        tier: models::SponsorshipTier,
    ) -> WebhooksSponsorship {
        WebhooksSponsorship {
            created_at,
            maintainer: None,
            node_id,
            privacy_level,
            sponsor: if let Some(x) = sponsor {
                Some(Box::new(x))
            } else {
                None
            },
            sponsorable: if let Some(x) = sponsorable {
                Some(Box::new(x))
            } else {
                None
            },
