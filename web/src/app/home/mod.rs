            ...
            Ok(repositorys
                ...
                .map(|(_, iter)| {
                    ...
                    let user = iter
                        ...
                        .clone();
                    let repos = iter
                        .sorted_by_key(|(r, _)| r.name.clone())
                        .map(|(repo, _)| repo)
                        .collect::<Vec<_>>();
                    (user, repos)
                })
                .sorted_by_key(|(u, _)| u.as_ref().map(|u| u.login.clone()))
                .collect::<Vec<_>>())
        },
    );
    return move || {
        let repositorys_by_owner = match (**repositorys_by_owner.read()).clone().transpose()? {
            Some(r) => r,
            None => return Ok(None),
        };
        Ok::<_, FrontendError>(Some(view! {
            ...
            <aside id="sidebar-multi-level-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
               ...
               <For
                    each=move || repositorys_by_owner.clone()
                                ...
                                <ul id="dropdown-example" class="py-2 space-y-2">
                                    <For
                                        ...
                                        children=move |repo| {
                                            ...
                                            view! {
                                                <li>
                                                    <a href=href class="flex items-center w-full m-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">{repo.name}</a>
                                                </li>
                                            }
                                        }
                                    />
